...
The Design System provides some utilities to write stable unit tests. All functions return a promise and cand be fullfilled with `await`.
...
| `waitForComponent(el: HTMLElement)`              | Waits until the compoennt and his nested components has rendered.                                                                                        |
...
and firt browser print has fullfilled. Moreover, it disables all the animations for more stable
and faster tests.
