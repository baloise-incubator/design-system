  ...
  function visualSelectTest(platform: 'mobile' | 'desktop') {
    describe(platform, () => {
      ...
      it('disabled component', () => {
        cy.getByTestId('disabled').wait(100).compareSnapshot(`select-disabled-${platform}`, 0.0)
      })

      it('invalid component', () => {
        cy.getByTestId('invalid').wait(100).compareSnapshot(`select-invalid-${platform}`, 0.0)
      })

      it('field component', () => {
        cy.getByTestId('field').wait(100).compareSnapshot(`select-field-${platform}`, 0.0)
      })
    })
  }
  ...
  function visualSelectMultipleTest(platform: 'mobile' | 'desktop') {
    describe(platform, () => {
      ...
      it('disabled multiple component', () => {
        cy.getByTestId('disabled').wait(100).compareSnapshot(`select-multiple-disabled-${platform}`, 0.0)
      })

      it('invalid multiple component', () => {
        cy.getByTestId('invalid').wait(100).compareSnapshot(`select-multiple-invalid-${platform}`, 0.0)
      })

      it('field component', () => {
        cy.getByTestId('field').wait(100).compareSnapshot(`select-multiple-field-${platform}`, 0.0)
      })
    })
  }
})
